...
function recordRestChannelOutboundActivity(accountId: string): void {
  try {
    getRestChannelRuntime().channel.activity.record({
    ...
    if (
      ...
      error.message !== "REST channel runtime not initialized"
    ) {
      throw error;
    }
  }
...
export async function sendMessageRestChannel(
  ...
  const cfg = (opts.cfg ?? getRestChannelRuntime().config.loadConfig()) as CoreConfig;
  ...
  try {
    if (!response.ok) {
      ...
      if (status === 401) {
        errorMsg = "REST channel: authentication failed — check outboundApiKey or outboundBearerToken";
      } else if (status === 403) {
    ...
    recordRestChannelOutboundActivity(account.accountId);
